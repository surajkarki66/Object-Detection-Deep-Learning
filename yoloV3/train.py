...
from model.yolov3 import complete_yolov3, decode, YOLOv3
...
def main():
    ...
    if TRAIN_TRANSFER:
        Darknet = complete_yolov3(input_size=YOLO_INPUT_SIZE)
        load_yolo_weights(Darknet, Darknet_weights) # use darknet weights
        #load_tiny_yolo_weights(Darknet, Darknet_weights) # use darknet weights

    yolo = complete_yolov3(input_size=YOLO_INPUT_SIZE, training=True, CLASSES=TRAIN_CLASSES)
    ...
    def train_step(image_data, target):
        with tf.GradientTape() as tape:
            ...
            for i in range(grid):
                ...
                loss_items = loss(pred, conv, *target[i], i, CLASSES=TRAIN_CLASSES)
